...
> 벽 보간·경로 계산·Marker 퍼블리시는 **항상 수행**
> 하지만 조향각과 PWM 값 퍼블리시는 `mission_state == 2` 조건으로 gating.
"""
import math
...
class RightWallBSplinePointsPublisher(MissionSubscriberMixin):
    ...
    def process_scan(self, scan_msg: LaserScan):
        ...
        # DBSCAN 클러스터링
        labels = DBSCAN(eps=0.1, min_samples=5).fit_predict(pts_np)
        uniq = set(labels) - {-1}
        if not uniq:
            return
        ...
        for lab in uniq:
            ...
            if any(85 <= math.degrees(remap_angle(p)) <= 95 for p in cl):
...
class FinalPathPublisher(MissionSubscriberMixin):
    def __init__(self):
        ...
        self.max_Ld    = 1.2
        self.min_Ld    = 0.3
        self.rear_axle = np.array([-0.2, 0.0])
    ...
    def timer_cb(self):
        ...
        if self.in_mode_b():
            ...
            pwm = int(self.max_pwm - (min(abs(wheel_deg), 20.0) / 20.0) * pwm_range)
