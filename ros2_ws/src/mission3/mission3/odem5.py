...
• mission_state == 3(Mode C)에 진입하면, 두 토픽이 **모두**
  수신된 이후에 미션이 시작된다. 그 전에는 대기.

• Waypoint 는 Look-ahead 간격으로 등분할 생성하며,
  항상 **현재 인덱스 이후**의 Waypoint 만 추종한다.

+ LiDAR yaw 수신 시점의 엔코더 카운트를 저장해
  미션 시작 기준점으로 사용.
+ 중간점(1,0)과 최종 목적지 모두 “반경(톨러런스)”으로
  도달 판정한다.
"""
...
class Mission3Node(Node):
    def __init__(self):
        ...
        self.declare_parameter("mid_radius",   0.30)    # ★ 중간점 반경
        ...
        # LiDAR yaw 수신 시점 엔코더 기준점
        self.encoder_at_lidar = None

        # 경로 & Waypoint 인덱스
        self.path              = []
        self.current_wp_idx    = 0
        self.mid_idx           = None         # ★ 중간점 인덱스
        self.mid_pt            = None         # ★ (1,0)
        self.mid_passed        = False        # ★
    ...
    def start_mission(self):
        ...
        # ──────★ 경로 생성 (start → mid → goal) ──────
        gx = self.get_parameter("goal_x").value
        gy = self.get_parameter("goal_y").value
        ld = self.get_parameter("lookahead").value

        mid_x, mid_y = 1.0, 0.0                         # ★ 중간 목표
        self.mid_pt  = (mid_x, mid_y)                   # ★

        path = []

        # (a) 시작 → 중간
        dist1 = math.hypot(mid_x - sx, mid_y - sy)
        n1    = max(int(dist1 / ld), 1)
        path += [(sx + (mid_x - sx) * i / n1,
                  sy + (mid_y - sy) * i / n1)
                 for i in range(1, n1 + 1)]             # 시작점 제외
        self.mid_idx = len(path) - 1                    # ★ mid 인덱스

        # (b) 중간 → 최종
        dist2 = math.hypot(gx - mid_x, gy - mid_y)
        n2    = max(int(dist2 / ld), 1)
        path += [(mid_x + (gx - mid_x) * i / n2,
                  mid_y + (gy - mid_y) * i / n2)
                 for i in range(1, n2 + 1)]             # 중복 피하려 mid 제외
    ...
    def timer_cb(self):
        ...
        # (2-a) 중간점 반경 판정 ★
        if not self.mid_passed:
            mx, my = self.mid_pt
            if math.hypot(mx - self.x, my - self.y) <= self.get_parameter("mid_radius").value:
                self.mid_passed = True
                self.get_logger().info("중간점(1,0) 통과 완료")
        ...
        ld = self.get_parameter("lookahead").value
        # ★ mid 통과 전이면 mid_idx까지만 탐색
        limit_idx = len(self.path) if self.mid_passed else (self.mid_idx + 1)
        for idx in range(self.current_wp_idx, limit_idx):
            px, py = self.path[idx]
            ...
            # 후보 없음 → 마지막 허용 인덱스 사용
            tx, ty = self.path[limit_idx - 1]
            self.current_wp_idx = limit_idx - 1
